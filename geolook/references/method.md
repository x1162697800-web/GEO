...
**差异封锁只认按身份拒绝的状态码。** `401/403/406/451` 才算 `ai_ua_blocked`；
`429/503` 是速率/容量信号，跟 UA 无关，任何客户端打太快都会撞上——探测本身对同一页连发
数个请求，很容易自己把自己打成 429。所以速率类响应先退避重试，仍是速率类就归入
`ai_ua_rate_limited`（判为**未测**，不是通过也不是封锁），并用默认 UA 打一次基线：
基线也被限速时整轮探测不可结论。这条纪律是有代价的——把限速误报成「WAF 封了所有 AI
爬虫」会让用户去动 WAF，而按第 7 节的风险分级那是最高风险的改造。
