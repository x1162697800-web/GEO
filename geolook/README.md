...
|---|---|
| `llms.txt` | your site root |
| JSON-LD | into `<head>`, by page type |
| Definition & FAQ snippets | into page templates |
| Content outlines | to the writer, never online |
...
| **Diagnosis depth** | A flat checklist | Four dependent layers (Access → Orientation → Understanding → Quotability) that separate **eligibility gates from influence levers**, plus passage-level quotability — retrieval picks passages, not pages |
...
- `scripts/service.sh` (macOS launchd service) and `extension/sandbox.sh`
  (one-command clean-browser sandbox) are POSIX-only. Without `sandbox.sh` you
  open an incognito window manually to keep sampling hygiene — see
  [references/method.md](references/method.md) §4.
- `.env` is written with `chmod 0o600` on POSIX so other local users can't read
  your keys. **Windows `chmod` can't express that, so this protection does not
  apply** — restrict the file with NTFS permissions if you share the machine.
...
# 2. Start the customer site (the product)
python3 scripts/geo.py online    # → http://127.0.0.1:8787
#    Consultant workbench: python3 scripts/geo.py ui  → http://127.0.0.1:8765
...
**Engine access is provisioned at deployment.** Whoever deploys fills in `.env` once; the person using it never configures a key. Run `geo.py doctor` before handing an install over — it verifies the customer app, engine reachability, dependencies and file permissions, and tells you exactly what is missing.
...
Eight tools: `list_projects`, `get_site_audit`, `get_brand_facts`, `list_tickets`, `get_question_bank`, `get_method_reference`, `run_audit`, `generate_assets`. Hand-written stdio JSON-RPC — no new dependencies.

Deliberate boundaries: **publishing is never exposed** (every publish stays an explicit human click), crawling is not exposed either (agent-driven network side effects on third-party sites), and project slugs go through a whitelist. The two write tools (`run_audit`, `generate_assets`) only touch `work/` and never reach the network.

Both are optional — every script is a plain CLI.
