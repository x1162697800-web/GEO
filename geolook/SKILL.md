...
description: >-
  ...
  做站点诊断与 AI 答案采样、生成带验收标准的执行工单、把一份品牌事实卡编译成
  六个部署目标（llms.txt / JSON-LD / 定义块与 FAQ 片段 / 内容大纲 / AI 流量归因包 /
  装进 agent 的品牌 SKILL.md）、自动验收工单是否闭环、
...
3. **robots / WAF 有没有拦 AI 抓取器**——`ai_bots_blocked` 是 robots 封禁（含通配符组），
   ...
   `ai_ua_rate_limited` 是撞上站点限速（429/503）导致**没测出结果**，不是封锁——
   不要据此建议改 WAF，重跑抓取再看
