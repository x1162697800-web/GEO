...
|---|---|
| `llms.txt` | サイトルート |
| JSON-LD | ページ種別ごとに `<head>` へ |
| 定義ブロック / FAQ 断片 | ページテンプレートへ |
| コンテンツアウトライン | ライター向け・非公開 |
...
| **診断の深さ** | フラットなチェックリスト | 四層依存チェーン（アクセス → オリエンテーション → 理解 → 引用適性）が**参加資格ゲートと影響力レバーを分離**。加えて段落レベルの引用適性——検索はページではなく段落を選ぶ |
...
| **コスト** | サブスクリプション | 無料・オープンソース。エンジン接続はデプロイ時に用意されるため、自分で契約も設定も不要。未接続の環境でも手動サンプリングで動く |
...
Windows では 2 点が異なります：

- `scripts/service.sh`（macOS の常駐サービス）と `extension/sandbox.sh`
  （クリーンブラウザ・サンドボックスをワンコマンドで起動）は POSIX 専用です。
  `sandbox.sh` がない場合はシークレットウィンドウを手動で開いてサンプリング衛生を
  保ってください（[references/method.md](references/method.md) §4）。
- POSIX では `.env` を `chmod 0o600` で書き込み、同一マシンの他ユーザーが
  鍵を読めないようにしています。**Windows の `chmod` はこれを表現できないため、
  この保護は適用されません**——マシンを共有する場合は NTFS 権限で制限してください。
...
# 3. エンジン接続を用意 —— デプロイ側の作業。利用者のタスクではない
#    cp .env.example .env にキーを記入し、検証：
python3 scripts/geo.py doctor
```

**エンジン接続はデプロイ時に用意されます。** Grounded をデプロイする人が `.env` を一度記入し、使う人はキーに一切触れません。引き渡す前に `geo.py doctor` を実行——エンジン到達性・依存・ファイル権限・ポートを検証し、何が不足しているかを明示します。

**エンジンなしでも動きます**：クロール・サイト監査・チケット・デプロイ資産はエンジンを一切必要とせず、手動サンプリングシートは自動サンプリングと同一の指標を生成します。エンジン接続が必要なのは自動サンプリングと AI 推論による質問バンク生成のみです。
