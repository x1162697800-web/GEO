...
class _Server(ThreadingHTTPServer):
    """Windows 的 SO_REUSEADDR 语义与 POSIX 不同：它允许第二个进程绑上一个
    **正在监听**的端口（POSIX 只允许复用 TIME_WAIT）。默认的 allow_reuse_address=True
    因此会让看板在端口被占时「启动成功」却服务不了请求——首页可能还是别人的应用。
    POSIX 上保留 True，否则每次重启都会撞 TIME_WAIT。"""
...
def run(port: int = 8765, open_browser: bool = True,
    ...
    if _port_taken(host, port):
        G.die(f"端口 {port} 已被占用（可能是另一个看板或别的应用）。"
              f"换个端口启动：python3 scripts/geo.py ui --port {port + 1}")
    Handler.TOKEN = token
