...
# 差异封锁的证据：按身份拒绝。换 UA 就通说明拦的是「你是谁」。
UA_DENY_STATUS = (401, 403, 406, 451)
# 速率/容量信号：跟 UA 无关，任何客户端打太快都会撞上，不能当封锁读
UA_RATE_STATUS = (429, 503)


def probe_ai_ua(root: str, home: dict, robots_txt: str,
                delay: float) -> tuple[dict, list[str], list[str]]:
    """换 AI 爬虫 UA 抓一次首页，检出 WAF/CDN 的差异封锁。
    只对 robots 放行的爬虫探测（robots 都封了的，被 WAF 拦是站长本意，不算问题）；
    普通 UA 拿不到 200 时也不探测，那是站点本身的问题，不是差异封锁。

    返回 (probe, ua_blocked, rate_limited)。**429/503 绝不算差异封锁**：
    本函数对同一个首页连发数个请求，很容易自己把自己打成 429，而
    「WAF 封了所有 AI 爬虫」是 P0 级结论，会让用户去动 WAF（高风险改造）。
    所以速率类响应先退避重试一次，仍是速率类就归入 rate_limited（未测），
    并用默认 UA 打一次基线：基线同样被限速时，整轮探测判为不可结论。"""
    probe: dict[str, int] = {}
    ua_blocked: list[str] = []
    ...
    for bot, ua in AI_UA_PROBES.items():
        ...
        if res["status"] in UA_RATE_STATUS:
            time.sleep(max(delay, 2.0) * 2)
            res = G.fetch(root, timeout=10, retries=0, ua=ua)
        probe[bot] = res["status"]
        if res["status"] in UA_DENY_STATUS:
            ua_blocked.append(bot)
        elif res["status"] in UA_RATE_STATUS:
            rate_limited.append(bot)
        time.sleep(delay)
    # 基线对照：默认 UA 此刻也被限速，说明拿到的拒绝是站点在限流而非按身份挑人，
    # 这一轮的 ua_blocked 不可信，整体降级为「未测」
    if ua_blocked or rate_limited:
        time.sleep(max(delay, 1.0))
        base = G.fetch(root, timeout=10, retries=0)
        probe["_baseline"] = base["status"]
        if base["status"] in UA_RATE_STATUS or base["status"] == 0:
            rate_limited = sorted(set(rate_limited) | set(ua_blocked))
            ua_blocked = []
    return probe, ua_blocked, rate_limited
...
def run(slug: str, max_pages: int | None = None, delay: float = 0.5) -> dict:
    ...
    ua_probe, ua_blocked, ua_rate_limited = probe_ai_ua(root, home, robots_txt, delay)
    ...
    site = {
        ...
        "ai_ua_rate_limited": ua_rate_limited,
    ...
    if ua_rate_limited:
        G.info(f"注意：{'、'.join(ua_rate_limited)} 的 UA 探测撞上站点限速（429/503），"
               "这不是差异封锁——结果按「未测」记，稍后单独重跑抓取再看")
