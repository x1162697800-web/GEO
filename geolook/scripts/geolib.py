...
只依赖 requests / bs4 / lxml，不引入 yaml/jinja2。
"""
...
# 文件锁：POSIX 用 fcntl.flock，Windows 用 msvcrt 的字节区间锁。
# 两者语义一致——独占、阻塞等待、进程退出时由内核释放（所以崩溃不会留下死锁）。
try:
    import fcntl
...
def _lock_acquire(fd, slug: str):
    """拿独占锁，阻塞到成功。不设超时——与 flock 语义对齐（持锁进程退出即释放）。"""
    if fcntl is not None:
        fcntl.flock(fd, fcntl.LOCK_EX)
        return
    # msvcrt 没有无限阻塞模式（LK_LOCK 只重试 10 次就抛 OSError），所以自己轮询。
    # 锁固定的第 0 字节：区间必须跨进程一致，锁到 EOF 之外是允许的。
    fd.seek(0)
    tries = 0
    while True:
        try:
            ...
            tries += 1
            if tries == 100:  # 约 10 秒还没拿到：说明真在等别人，报一声但继续等
                info(f"等待项目 {slug} 的锁释放：另一个任务正在写这个项目")
            time.sleep(0.1)
...
def project_lock(slug: str):
    ...
    锁文件用 "a+" 打开而不是 "w"：Windows 上别的进程持锁时截断会失败，
    而锁文件本来就不需要写内容。
    """
    d = project_dir(slug)
    d.mkdir(parents=True, exist_ok=True)
    with (d / ".lock").open("a+") as fd:
        _lock_acquire(fd, slug)
        try:
            yield
        finally:
            _lock_release(fd)
