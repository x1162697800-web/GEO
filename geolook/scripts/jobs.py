...
def _kill_tree(pid: int) -> bool:
    """杀掉整棵进程树。管线命令会再派生子进程，只杀父进程会留下孤儿继续写
    work/<slug>/，而此时并发保护已经放开——那正是 project_lock 要防的事。"""
    if IS_WINDOWS:
        # Windows 没有进程组信号，用 taskkill /T 递归收树
...
def _alive(pid: int) -> bool:
    ...
    Windows 上不能用 os.kill(pid, 0) 探活：Python 只把 CTRL_C_EVENT /
    CTRL_BREAK_EVENT 当信号处理，其他值一律走 TerminateProcess——
    也就是会把目标进程直接杀掉。必须用 OpenProcess + GetExitCodeProcess。
    """
    if IS_WINDOWS:
        import ctypes
        ...
        try:
            ...
            if not k32.GetExitCodeProcess(h, ctypes.byref(code)):
                return True  # 查不到状态就保守当存活：宁可漏收孤儿，不可误杀在跑的任务
...
def start(slug: str, action: str, params: dict | None = None) -> dict:
    ...
    try:
        proc = subprocess.Popen(cmd, stdout=logf, stderr=subprocess.STDOUT,
                                cwd=str(G.ROOT), env=env, **_spawn_kwargs())
...
def stop(job_id: str) -> bool:
    ...
    if proc:
        try:
            if not _kill_tree(proc.pid):
                G.info(f"收进程树失败，回落到只终止父进程：pid={proc.pid}")
                proc.terminate()
        except Exception as e:  # noqa: BLE001
            G.info(f"收进程树异常（{type(e).__name__}），回落到只终止父进程")
            proc.terminate()
        return True
    # 服务重启后 _procs 是空的，按 job 文件里落的 pid 兜底杀整棵树
    job = get(job_id)
    pid = (job or {}).get("pid")
