...
def cmd_detect(a):
    ...
    if not cfg.get("questions"):
        ...
        try:
            platforms = [x for x in (getattr(a, "platforms", "") or "").split(",") if x]
            metrics = S.run(a.slug, platforms=platforms or None,
                            repeat=getattr(a, "repeat", 1), limit=a.limit)
...
def main():
    ...
    s.add_argument("--slug", required=True)
    s.add_argument("--asset", help="逗号分隔：llms,jsonld,snippets,outlines,attribution,skill")
    ...
    s = sub.add_parser("doctor", help="交付前置自检（客户网站/引擎/权限/端口）")
