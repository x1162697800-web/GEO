...
| **诊断深度** | 一张平铺的检查清单 | 四层依赖链（访问 → 定向 → 理解 → 可引用）把**门票判据和影响力杠杆分开**，外加段落级可引判定——检索是按段落选材的，不是按页面 |
...
| **成本** | 按月订阅 | 开源免费。引擎接入在部署时配好，你不需要自己买 key、也不需要配置；没接引擎的场景用人工采样表照样能跑 |
...
- `scripts/service.sh`（macOS 常驻服务）和 `extension/sandbox.sh`（一键干净浏览器沙箱）
  是 POSIX 专属。没有 `sandbox.sh` 就手动开无痕窗口来保证采样卫生，
  见 [references/method.md](references/method.md) 第 4 节。
- POSIX 上 `.env` 会以 `chmod 0o600` 写入，同机其他用户读不到你的密钥。
  **Windows 的 `chmod` 表达不了这个权限，所以这层保护不生效**——
  与他人共用机器时请用 NTFS 权限自行限制该文件。
...
- 「部署资产」把 llms.txt 传网站根目录、JSON-LD 贴进 `<head>`、片段贴进模板、`SKILL.md` 装进 Claude / Codex / Cursor 的 skills 目录（每个文件顶部写了去处，完整清单见 DEPLOY.md）
